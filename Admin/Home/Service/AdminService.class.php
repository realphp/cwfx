...
class AdminService extends CommonService {
    ...
    public function add($admin) {
        ...
        $Admin->startTrans();
        $newAdmin = array(
            'email' => $admin['cnum'].'@qq.com',
            'address' => '无',
            'kind' => 1,
        );
        $newuser = array_merge($admin, $newAdmin); 
        if (false === ($admin = $Admin->create($newuser))) {
    ...
    public function login($admin) {
        ...
        //是否拥有进后台的权限
        if (!$this->isAdministrator($admin['email'])) {
            return $this->errorResultReturn('你不是管理员，不能进入后台！');
        }
