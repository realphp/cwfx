...
class AdminService extends CommonService {
    ...
    public function login($admin) {
        ...
        $account = $Admin->getByUsername($admin['username']);
        // 密码验证
        ...
        // 是否启用
        if (!$this->isActive($admin['username'])) {
            return $this->errorResultReturn('账户已被禁用！');
        }
        //是否拥有该软件使用权限
        if(!$this->isKind($admin['username'])){
            return $this->errorResultReturn('该账户没有使用权限！');
        }
        ...
        if (C('USER_AUTH_ON')) {
            ...
            if ($account['is_super']=='0'||$account['is_super']=='1') {
                // 超级管理员无需认证
                $_SESSION[C('ADMIN_AUTH_KEY')] = true;
//                var_dump($_SESSION);
//                if($account['is_super']==0){
//                    $_SESSION[C('ADMIN_AUTH_KEY')] = true;
//                }
            }
    ...
    /**
     * 查询账户下子账户数量
     * @param  string  $cnum cnum
     * @return 
     */
    public function getCnum($cnum) {
//        $cnum = '1001';
        $admin = $this->getM()->getByCnum($cnum);
//       $sql = $this->getM() ->getLastSql();
//       echo $sql;
        return $admin;
    }
     /**
     ...
     * @return 
     */
    public function getNowAdmin($cnum) {
//        $cnum = '1001';
        $num = $this->getM()->where("cnum='{$cnum}'")->count();
//       $sql = $this->getM() ->getLastSql();
//       echo $sql;
        return $num;
    }
     ...
     * @return 
     */
    public function getNowCompany($cnum) {
        $result = $this->getM()->where("cnum='{$cnum}'")->select();
        return $result;
    }
     ...
     * @return boolean
     */
    public function isKind($username) {
        $where = array(
            'username' => $username,
            'kind' => 1
        );
